...
import { activarDesactivarData, fetchData, fetchResumen, guardarDataEditado, guardarNuevaData } from "@/components/utils/httpTipoHabitacion"
...
export default function TipoHabitacionesPage() {
  ...
  const [dataDetalle, setDataDetalle] = useState<TipoHabitacion>();
  ...
  const {data, isFetching, isError} = useQuery({
    queryKey: ['tipo-habitaciones', currentPage, paginacion.pageSize, nombreABuscar, showActiveOnly], //data cached
  ...
  const {data: dataResumen, isFetching: isFetchingResumen, isError: isErrorResumen} = useQuery({
    queryKey: ['tipo-habitaciones-resumen'], //data cached
    queryFn: () => fetchResumen(),
    staleTime: 5 * 60 * 1000 //despues de 5min los datos se consideran obsoletos
  });

  // let filteredPermissions: Modulo[] = [];
  let dataList: TipoHabitacion[] = [];

  if(!isFetching && !isError){
    dataList = data.results.map((per: TipoHabitacion, index: number) => ({...per, numero: index + 1}));
  }
  ...
  const {mutate, isPending: isPendingMutation} = useMutation({
    mutationFn: guardarNuevaData,
    onSuccess: () => {
        handleShowToast('Se ha creado un nuevo tipo de habitacion satisfactoriamente', 'success');
        reset({
            nombre: "",
            capacidad: 2,
          });
        setActiveTab('list');
         ...
         queryClient.invalidateQueries({
          queryKey: ['tipo-habitaciones'],
          exact: false
        });

        queryClient.invalidateQueries({
          queryKey: ['tipo-habitaciones-resumen'],
        });
    },
  });
  ...
  const {mutate: mutateGuardarEditado, isPending: isPendingEdit} = useMutation({
    ...
    onSuccess: () => {
        handleShowToast('Se ha guardado el tipo de habitacion satisfactoriamente', 'success');
  ...
  const {mutate: mutateDesactivar, isPending: isPendingDesactivar} = useMutation({
    ...
    onSuccess: () => {
        handleShowToast(`Se ha ${textActivarDesactivar} el tipo de habitaciones satisfactoriamente`, 'success');
  ...
  useEffect(() => {
    if (dataAEditar) {
      reset({
        ...
        capacidad: dataAEditar.capacidad,
      });
    }
  }, [dataAEditar, reset]);


  const handleEditar = (data: TipoHabitacion) => {
    setActiveTab('form');
    setDataAEditar(data);
  }

  const toggleActivar = (modulo: TipoHabitacion) => {
  ...
  const handleVerDetalles = (data: TipoHabitacion) => {
    setDataDetalle(data);
    setOnVerData(true);
  }
  ...
  return (
    <>
                      ...
                      <div className="mb-6 border-b pb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">Detalles completos del tipo de habitacion</p>
                          </div>
                        </div>
                      </div>
            ...
            <div className="modal-detalles-reserva bg-white/95 rounded-xl shadow-xl max-w-7xl w-full max-h-[95vh] overflow-y-auto backdrop-blur-sm">
              <Modal onClose={handleCloseModal} claseCss="modal">
                    ...
                    <p className=' text-gray-600 dark:text-gray-400 mt-2 text-justify'>
                      ¿Estás seguro de que deseas {dataADesactivar!.activo ? 'desactivar' : 'activar'} el tipo de habitacion <b>{capitalizePrimeraLetra(dataADesactivar?.nombre ?? '')}</b>? 
                    </p>
      ...
      <div className="max-w-[95vw] mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 bg-blue-500 rounded-xl flex items-center justify-center">
                   {/* <IoDocumentSharp className="h-5 w-5 text-white" /> */}
                   <MdMeetingRoom className="h-6 w-6 text-white" />
                </div>
                <h1 className="text-3xl font-semibold text-gray-900">Tipo Habitaciones</h1>
              </div>
              <p className="text-gray-600">Gestiona los tipos de habitaciones del sistema y sus estados.</p>
            </div>
            {/* <div className="flex gap-3">
              ...
              */}
              ...
              {siTienePermiso("tipos_habitaciones", "crear") && 
          ...
          {/* Stats Cards */}
          <ResumenCardsDinamico 
                resumen={dataResumen}
                isFetchingResumen={isFetchingResumen}
                isErrorResumen={isErrorResumen}
            />
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="grid w-full grid-cols-2 lg:w-fit bg-gray-100">
              ...
              <TabsTrigger 
                disabled={!siTienePermiso("tipos_habitaciones", "leer")}
                ...
                <Card className="border-emerald-200 pt-0">
                  <CardHeader className="bg-emerald-50 border-b border-emerald-200 pt-8">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-emerald-900">Crear Nuevo Tipo Habitacion</CardTitle>
                        <CardDescription className="text-emerald-700">
                          Complete la información para crear un nuevo tipo de habitacion
                        </CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Input
                          id="nombre"
                          autoComplete="nombre"
                          placeholder="Doble, Doble Standar, Doble Vista al Mar, etc"
                      ...
                      <div className="space-y-2 mt-5">
                        {/* <Label htmlFor="capacidad" className="text-gray-700 font-medium">
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                              <div className="select-container"> {/* Contenedor para controlar el layout */}
                                ...
                                >
                                  <SelectTrigger className="w-full cursor-pointer border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-left !h-[2.35rem]">
                                    <SelectValue placeholder="Moneda" />
                                  </SelectTrigger>
                                  <SelectContent className="min-w-[var(--radix-select-trigger-width)] max-h-60">
                                    {CAPACIDAD_PERSONAS_LIST.map((item: string, index: number) => 
                                      <SelectItem 
                                        key={index}
                ...
                <CardHeader className="bg-blue-50 border-b border-blue-200 pt-8">
                  <div className="flex items-center justify-between ">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-blue-900">Lista de Tipo Habitaciones</CardTitle>
                        <CardDescription className="text-blue-700">
                          Mostrando {startIndex + 1}-{Math.min(endIndex, totalItems)} de {totalItems} tipos de habitaciones
                        </CardDescription>
                      </div>
                    </div>
                ...
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody className="w-full">
                      ...
                      {!isFetching && dataList.length > 0 && siTienePermiso("tipos_habitaciones", "leer") && dataList.map((data: TipoHabitacion) => (
                        <TableRow
                          key={data.id}
                          ...
                          <TableCell>
                            <div>
                              ...
                              <div className="text-sm text-gray-500 truncate max-w-xs">
                                {data.capacidad === 1 ? '1 persona': data.capacidad + ' personas'}
                              </div>
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="border-gray-200">
                                {siTienePermiso("tipos_habitaciones", "leer") &&
